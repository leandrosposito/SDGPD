...
### Definition of Done
- [ ] Los parámetros del sistema pueden leerse y modificarse desde la interfaz, y los cambios se reflejan inmediatamente en los módulos que los consumen.
...
- **Criterios de aceptación**: Un usuario con rol "Vendedor" no debe poder acceder a las pantallas de Configuración ni ejecutar operaciones de Administrador.
...
- **Criterios de aceptación**: Al bloquear un usuario con sesión activa, su próximo request debe ser rechazado con error de autenticación.
...
- **Criterios de aceptación**: Al activar la política estricta, el sistema debe rechazar contraseñas que no cumplan la complejidad mínima configurada.
...
### Definition of Done
- [ ] Las políticas de contraseñas configuradas se aplican correctamente al crear o cambiar claves, y las sesiones caducan según el timeout definido.
...
- **Criterios de aceptación**: Al asignar un límite de crédito de $1000 a un cliente con condición Cta Cte, el sistema debe almacenarlo y utilizarlo como tope en la validación de pedidos.
...
- **Criterios de aceptación**: La ficha 360 de un cliente con historial transaccional debe cargarse en menos de 2 segundos y mostrar los últimos pedidos, pagos y saldo actual.
...
- **Criterios de aceptación**: Un cliente bloqueado por morosidad no debe poder generar nuevos pedidos, pero sí debe poder registrar pagos.
...
- **Criterios de aceptación**: Un remito generado con dirección de entrega secundaria debe mostrar esa dirección y no el domicilio fiscal del cliente.
...
- **Criterios de aceptación**: Al filtrar clientes por zona logística, solo deben aparecer los clientes asignados a esa zona.
...
- **Flujo principal**: 1. Administrador accede a Proveedores. 2. Crea nuevo proveedor con datos fiscales (CUIT, Razón Social, Condición IVA). 3. Asigna tipificación (Bienes/Servicios). 4. Sistema valida unicidad de CUIT. 5. Guarda.
- **Reglas de negocio**: CUIT único.

- **Criterios de aceptación**: Al crear un proveedor con CUIT válido y condición fiscal, el sistema debe persistirlo y permitir seleccionarlo en una Orden de Compra.
...
- **Criterios de aceptación**: Al crear una Orden de Compra, el plazo de pago debe heredarse automáticamente del proveedor seleccionado.
...
- **Criterios de aceptación**: Al bloquear un proveedor, el sistema debe impedir la creación de nuevas Órdenes de Compra dirigidas a ese proveedor.
...
- **Criterios de aceptación**: Al enviar una Orden de Compra por email, el sistema debe seleccionar automáticamente el contacto del proveedor con rol de Ventas.
...
- **Criterios de aceptación**: No debe ser posible eliminar una categoría que tenga productos asociados. El sistema debe mostrar un mensaje indicando la restricción.
...
- **Criterios de aceptación**: Al crear un producto con SKU y código de barras válidos, este debe poder buscarse por cualquiera de esos identificadores.
...
- **Criterios de aceptación**: Solo puede existir un proveedor principal por producto. Al marcar uno nuevo como principal, el anterior debe desmarcarse automáticamente.
...
- **Criterios de aceptación**: Al generar variantes combinatorias (ej: 3 talles x 2 colores), deben crearse exactamente 6 SKU hijos con stock independiente.
...
- **Criterios de aceptación**: Al recepcionar 1 Caja de 12 unidades, el stock debe incrementarse en 12 unidades base.
...
- **Criterios de aceptación**: Al facturar, el precio sugerido debe corresponder al cruce entre el producto y la lista de precios asignada al cliente.
...
- **Criterios de aceptación**: Al actualizar el costo de un producto, las listas de precios dinámicas vinculadas deben recalcular sus valores automáticamente.
...
- **Criterios de aceptación**: Al agregar 3 unidades de un producto con promoción "3x2", el total debe reflejar el descuento correspondiente a 1 unidad bonificada.
...
- **Criterios de aceptación**: La consulta de stock debe mostrar el stock físico por producto y por depósito, actualizado en tiempo real.
...
- **Criterios de aceptación**: Un ajuste negativo no debe poder ejecutarse si deja el stock físico del producto en valores negativos.
...
- **Criterios de aceptación**: No debe ser posible modificar ni eliminar un movimiento de inventario ya asentado.
...
- **Criterios de aceptación**: Al trasladar mercadería, el stock debe decrementarse atómicamente en el depósito origen e incrementarse en el destino.
...
- **Criterios de aceptación**: Al preparar un pedido, el sistema debe sugerir primero el lote con fecha de vencimiento más próxima (FEFO).
...
- **Criterios de aceptación**: Al cargar un conteo físico con diferencias, el sistema debe generar automáticamente los ajustes positivos o negativos correspondientes.
...
- **Criterios de aceptación**: Al confirmar un pedido que consume el 100% del stock, otro vendedor debe ver disponibilidad igual a cero para ese producto.
...
- **Criterios de aceptación**: No debe ser posible modificar una Orden de Compra que ya tiene recepciones parciales registradas.
...
- **Criterios de aceptación**: Al recibir 8 de 10 unidades pedidas, la OC debe quedar en estado Parcial y el stock debe incrementarse en 8.
...
- **Criterios de aceptación**: Al asentar la factura de compra, debe generarse automáticamente un movimiento deudor en la cuenta corriente del proveedor.
...
- **Criterios de aceptación**: Una solicitud de compra debe pasar por aprobación antes de poder convertirse en Orden de Compra.
...
- **Criterios de aceptación**: No debe ser posible devolver más cantidad de la originalmente recibida en la factura vinculada.
...
- **Criterios de aceptación**: Al asentar una factura de compra con precio unitario diferente al costo registrado, el costo del producto debe actualizarse automáticamente.
...
- **Criterios de aceptación**: Al generar un presupuesto, los precios deben calcularse según la lista de precios asignada al cliente seleccionado.
...
- **Criterios de aceptación**: Al autorizar un ticket de devolución, debe generarse un remito de retiro y, al recibir la mercadería, una nota de crédito.
...
- **Criterios de aceptación**: Si un pedido supera el límite de crédito del cliente, debe quedar en estado "Pendiente de Autorización" hasta aprobación gerencial.
...
- **Criterios de aceptación**: Una venta por mostrador debe generar atómicamente la factura, el cobro y la baja de stock físico.
...
- **Criterios de aceptación**: Al confirmar un pedido, el stock comprometido debe incrementarse y los precios deben quedar congelados.
...
- **Criterios de aceptación**: Si hay quiebre de stock durante la preparación, el sistema debe permitir despachar lo disponible y crear un backorder con el resto.
...
- **Criterios de aceptación**: Los pedidos atrasados deben visualizarse en color rojo en el panel de pedidos.
...
- **Criterios de aceptación**: Al cancelar un pedido no despachado, el stock reservado debe liberarse completamente. No debe ser posible cancelar un pedido ya despachado.
...
- **Criterios de aceptación**: Debe poder consultarse el historial completo de cambios de estado de un pedido, incluyendo usuario y timestamp.
...
- **Criterios de aceptación**: Las listas de picking deben agrupar ítems por zona de depósito para optimizar el recorrido del operario.
...
- **Criterios de aceptación**: El operario debe poder confirmar las cantidades realmente encontradas, y el sistema debe comparar contra lo solicitado.
...
- **Criterios de aceptación**: Al declarar un faltante, el stock físico debe ajustarse negativamente y el pedido debe particionarse automáticamente.
...
- **Criterios de aceptación**: El jefe de depósito debe poder asignar operarios a tareas de picking y visualizar el progreso en un panel.
...
- **Criterios de aceptación**: Al generar un viaje, los pedidos involucrados deben transicionar al estado Despachado.
...
- **Criterios de aceptación**: Las patentes de los vehículos deben ser únicas en el sistema.
...
- **Criterios de aceptación**: Al finalizar un viaje, el sistema debe notificar a la empresa y registrar el cambio de estado.
...
- **Criterios de aceptación**: Al confirmar la entrega, el pedido debe quedar en estado Entregado y bloqueado contra modificaciones posteriores.
...
- **Criterios de aceptación**: Al rechazar un pedido entregado, la mercadería debe reingresarse al stock físico y debe poder generarse la nota de crédito correspondiente.
...
- **Criterios de aceptación**: Al reprogramar una entrega, el remito debe desvincularse del viaje y volver a la cola de pendientes de asignación logística.
...
- **Criterios de aceptación**: Debe poder capturarse una foto o firma digital como evidencia de entrega, vinculada al pedido correspondiente.
...
- **Criterios de aceptación**: Al emitir una factura, el saldo deudor del cliente en su cuenta corriente debe incrementarse por el monto facturado.
...
- **Criterios de aceptación**: La suma de notas de crédito asociadas a una factura no debe superar el monto original de esa factura.
...
- **Criterios de aceptación**: Al generar una nota de débito, el saldo deudor del cliente debe incrementarse por el monto correspondiente.
...
- **Criterios de aceptación**: Al ejecutar facturación masiva, cada factura individual debe procesarse atómicamente; un error en una no debe afectar a las demás.
...
- **Criterios de aceptación**: El saldo deudor de un cliente debe reflejar correctamente la diferencia entre débitos (Facturas, ND) y créditos (Recibos, NC).
...
- **Criterios de aceptación**: Al aplicar un pago contra una factura, esta debe pasar a estado Pagada cuando el saldo pendiente sea cero.
...
- **Criterios de aceptación**: La cuenta corriente de proveedores debe reflejar las facturas de compra como pasivo y las órdenes de pago como cancelaciones.
...
- **Criterios de aceptación**: No debe ser posible guardar un recibo de cobro donde la suma de valores ingresados no coincida con la suma de documentos aplicados.
...
- **Criterios de aceptación**: Cada caja debe estar asociada a una sucursal y debe poder seleccionarse como destino al registrar cobros o pagos.
...
- **Criterios de aceptación**: Al emitir una orden de pago, el pasivo del proveedor en su cuenta corriente debe reducirse por el monto pagado.
...
- **Criterios de aceptación**: Al cerrar caja, si existe diferencia entre el conteo físico y el saldo del sistema, debe generarse automáticamente un ajuste.
...
- **Criterios de aceptación**: Al endosar un cheque de tercero para pagar a un proveedor, ese cheque debe dejar de aparecer en el arqueo de caja.
...
- **Criterios de aceptación**: Al facturar o cobrar, debe generarse automáticamente el asiento contable correspondiente según el mapeo de cuentas configurado.
...
- **Criterios de aceptación**: El reporte de flujo de fondos debe cruzar vencimientos de cuentas a cobrar, cuentas a pagar y saldos bancarios actuales.
...
- **Criterios de aceptación**: Los reportes de Libro IVA Ventas e IVA Compras deben generarse correctamente con los datos de facturas emitidas y recibidas.
...
- **Criterios de aceptación**: Todo listado del sistema debe poder exportarse a formato Excel o CSV.
...
- **Criterios de aceptación**: Debe poder consultarse la rentabilidad por línea de producto y el ranking de vendedores por período.
...
- **Criterios de aceptación**: Toda modificación o eliminación de entidades core debe quedar registrada con el estado anterior, el estado nuevo, el usuario y la fecha.
...
- **Criterios de aceptación**: Cada intento de login (exitoso o fallido) debe registrarse con IP, User-Agent y timestamp.
...
- **Criterios de aceptación**: Las notificaciones del sistema deben poder visualizarse en la interfaz (campanita) y enviarse por email de forma asíncrona.
...
- **Criterios de aceptación**: Cada usuario debe poder configurar qué tipos de notificaciones desea recibir.
...
- **Criterios de aceptación**: El sistema debe poder enviar una factura electrónica al ente fiscal y obtener el código de autorización (CAE) correspondiente.
...
- **Criterios de aceptación**: Al recibir un webhook de pago exitoso, el sistema debe generar automáticamente el recibo de cobro correspondiente.
...
- **Criterios de aceptación**: Las APIs públicas deben autenticarse mediante API Keys independientes de los usuarios físicos del sistema y contar con rate limiting.
