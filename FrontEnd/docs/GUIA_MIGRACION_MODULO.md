...
- [x] ~~`src/modules/inventory/InventoryPage.tsx` (tab Reposición / `TabPurchases`) — **Con `branchId`**.~~ **Migrado en Tanda 3f** (07/09/2026, commit `a68e6f4`). `api/` en `modules/inventory/api/purchase-suggestions/` (`dto.ts`, `mapper.ts`, `filterSort.ts`, `purchase-suggestions.service.ts`), verificado el 2026-09-09 — este checklist quedó sin tachar por error en una sesión anterior pese a que la migración sí se hizo.
...
**Verificación funcional en navegador PENDIENTE** para todas las tandas de migración de listados (el propio piloto `suppliers`, Tanda 2, 2.5, y 3a-3g) — ningún commit del historial confirma haberla ejecutado. El checklist de cada una vive en `docs/historial/verificaciones/VERIFICACION_TANDA_<id>.md`; ver `docs/ESTADO.md` para cuáles siguen sin evidencia de ejecución.
