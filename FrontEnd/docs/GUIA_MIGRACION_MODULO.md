...
**Verificación funcional en navegador PENDIENTE (de `suppliers`, el propio piloto) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_0_1.md`.** Hasta que esa verificación no esté confirmada, tratá el patrón de `suppliers` como "compila y compila el tipo, pero no probado en navegador todavía" al copiarlo — si el checklist de esa verificación encuentra un problema, corregilo en `suppliers` (la plantilla) ANTES de replicarlo a un módulo nuevo, no lo arrastres.
...
## Migrar un módulo que YA tenía service previo — lo aprendido en Tanda 3d (`clients`, Directorio)

Tandas 3a-3c migraron módulos que leían `data/mock/X.data.ts` directo en un `useState`, sin service de por medio. `clients` es el primer caso inverso: **ya tenía** `src/services/mock/clients.service.ts` desde Tanda 2.5 (ya pasaba por `httpClient`, el Directorio ya usaba `useCachedQuery`). Migrar "lo que falta" acá no es repetir los pasos de 3a-3c — son problemas distintos:

### 11. Antes de crear el `api/` nuevo, leé el service viejo COMPLETO buscando estado compartido entre funciones
Si el módulo ya tenía service, es muy probable que ese archivo tenga más de una función pública, y que alguna de ellas esté fuera del alcance de tu tarea (acá, `getClientAccountsPage`/`getOverdueClientsPage`, ya migrados en tandas previas). Antes de decidir dónde vive el código nuevo, leé el archivo completo — no solo las funciones que vas a tocar — buscando variables de módulo (`let`/`const` a nivel de archivo) que esas funciones fuera de alcance también usen. Si el store se comparte (`clientsStore` acá) y además hay algún mecanismo de cache/invalidación que dependa de la IDENTIDAD de esa variable (`overdueSnapshotCache.computedFor === clientsStore`, comparación por referencia, no por valor), separar el archivo en dos rompe ese mecanismo en silencio — sin ningún error de compilación que lo delate, porque TypeScript no puede detectar que dos variables `let` con el mismo shape en dos archivos distintos "deberían" ser la misma. La única forma de encontrar esto es leer el archivo entero, no confiar en que el nombre de la función que te interesa te dice todo lo que necesitás saber.

### 12. Si hay estado compartido intocable, absorbé el archivo completo — no lo dividas aunque la tarea te deje elegir
Si te dan libertad de decisión ("vos decidís si el service nuevo absorbe también las otras funciones o si quedan donde están"), tratá esa libertad como condicional a que no exista una razón técnica que la cierre. Acá la había (punto 11): la única forma de no romper la invalidación por referencia era mover TODO el archivo a la ubicación nueva, copiando la lógica fuera de alcance **100% literal, sin ningún cambio** — marcá cada bloque copiado con un comentario explícito ("COPIADO LITERAL desde X — prohibido tocarla") para que quede claro, tanto para vos como para quien lea el diff, qué es código nuevo y qué es una reubicación sin cambios. Verificá con un `diff` línea por línea contra el archivo viejo (vía `git show HEAD:ruta/vieja > /tmp/viejo.ts`, ya que el archivo se borra) que las únicas diferencias son comentarios — no lo des por sentado solo porque copiaste con cuidado.

### 13. Decidí el espacio del store (dominio vs. DTO) según qué lógica lo lee, no por consistencia con tandas anteriores
En `suppliers`/`orders`/`cash` (sin service previo) el store nuevo se guarda en forma DTO. Acá se decidió lo contrario: `clientsStore` quedó en forma de **dominio** (`ClientAccount[]`), porque la lógica FIFO/aging intocable (punto 12) lee `ClientAccount`/`ClientTransaction` directo, sin ninguna conversión — traducir el store a DTO hubiera obligado a traducir también esa lógica, que la tarea pedía no tocar. La conversión DTO↔dominio pasa a ocurrir solo en los bordes de las funciones que SÍ estás migrando (`getClientsPage`/`createClient`/`updateClient` acá), vía el `mapper.ts` nuevo. No hay una regla fija de "el store siempre va en DTO" — depende de si el código intocable que también lo usa espera dominio o DTO.

### 14. Un campo puede pasar por el DTO sin traducirse campo a campo, si nadie en el alcance de la tarea lo necesita traducido
`ClientAccountDTO.transacciones` es un pass-through verbatim del array de dominio (`ClientTransaction[]`), no una forma `snake_case` propia. El Directorio (lo único que este DTO sirve) nunca lee ni escribe ese campo — traducirlo hubiera sido trabajo sin ningún consumidor real, y hubiera ampliado el alcance hacia la lógica de imputación que la tarea pedía no tocar. Un DTO no tiene que traducir el 100% de sus campos con el mismo nivel de detalle; el criterio es qué necesita el consumidor real, no la pureza formal del contrato.

### 15. Un filtro que antes era gratis (en memoria) puede necesitar debounce que antes no tenía
El buscador de arriba ya viajaba debounced hacia Cuentas Corrientes/Morosos (migrados antes), pero el Directorio seguía recibiendo el valor sin debounce porque su filtrado en memoria era instantáneo y no había ningún costo en re-filtrar en cada tecla. Al mover el filtrado al service, ese mismo valor pasa a disparar una consulta real — si el módulo ya tenía un `debouncedSearchQuery` calculado para otra vista, usalo también acá en vez de dejar la vista migrada sin debounce por inercia del código viejo.
...
## Migrar una entidad a la que hay que AGREGARLE un campo de scope que no tenía — lo aprendido en Tanda 3g (`inventory`, Movimientos + Historial)

Todas las tandas anteriores (3a-3f) migraban un tipo de dominio que YA tenía todos los campos necesarios — el trabajo era de capa (dónde vive el código, DTO/mapper/service), nunca de modelo. `InventoryMovement`/`ProductHistoryEvent` fueron el primer caso real de lo contrario: el tipo de dominio no tenía ningún campo de sucursal, y sin él no había nada que un `api/` nuevo pudiera filtrar.

### 21. Si la entidad no tiene el campo de scope que necesitás, agregalo al TIPO primero — no hay forma de escribir el `api/` sin eso
Antes de tocar `dto.ts`, confirmá con `grep` si el campo de scope (acá, `branchId: Branch['id']`) ya existe en la interfaz de dominio (`shared/types/`). Si no existe, esto deja de ser una migración de capa pura: hay que decidir el razonamiento de scope (mismo criterio E1 que ya se usa en el resto del proyecto — ¿esto ocurre en un lugar físico concreto, o es del catálogo/empresa?), agregarlo a la interfaz, y — a diferencia de agregar un campo opcional a un formulario — **poblarlo en el mock**, porque los registros existentes no tienen ese dato. Documentá esto como AMPLIACIÓN DE MODELO en `DECISIONES_TECNICAS.md`, no como si fuera un refactor más — es una categoría de cambio distinta (agrega información que antes no existía, no solo la reorganiza).

### 22. Poblar el mock con un campo de scope nuevo no es "asignar cualquier valor" — tiene que ser verificable
Repartir los registros existentes entre los valores posibles del campo nuevo (acá, las 3 sucursales activas) de forma que NINGÚN valor concentre el 100% — si todos los movimientos quedan en `branch-001`, cambiar de sucursal en el checklist manual no prueba nada (siempre se ve lo mismo, o siempre vacío en las otras). Si el dataset original es muy chico para que la distribución sea demostrable (acá, 2 movimientos y 3 eventos de historial para 3 sucursales), sumar registros nuevos es parte legítima de la tarea — no hace falta pedir permiso para esto si ya se pidió "poblar el mock de forma verificable", pero sí hay que documentar cuántos son originales (con el campo agregado) y cuántos son nuevos, para que quede claro qué es dato preexistente y qué se inventó para poder probar la tanda.

### 23. Una "relación" entre dos entidades que dos registros de mock sugieren por coincidencia no es una relación modelada — no la asumas ni la inventes
Al leer los mocks de `movements`/`history`, un evento de historial y un movimiento describían el mismo hecho real (mismo SKU/fecha/cantidad) — fácil de leer como "un movimiento genera un evento de historial". Verificá contra el TIPO, no contra la narrativa de los datos de ejemplo: si no hay un campo que los vincule (acá, ningún `movementId`/`historyEventId` cruzado), no hay relación real que preservar ni que replicar en el `api/` nuevo — es una coincidencia de cómo se armó el mock a mano. Documentar esto explícitamente evita que una tanda futura "arregle" una relación que nunca existió, o que alguien asuma que ya está resuelta.
...
- [x] ~~`src/modules/clients/ClientsPage.tsx` (+ `components/ClientDirectoryTable.tsx`) — Directorio de Clientes.~~ **Migrado en Tanda 3d** (04/09/2026). Sin `branchId`, confirmado contra el código (M9, sin sorpresas). Primer módulo migrado que YA tenía service previo (`services/mock/clients.service.ts`, desde Tanda 2.5) — absorbido completo a `modules/clients/api/clients.service.ts` por un estado compartido (`clientsStore`) con `getClientAccountsPage`/`getOverdueClientsPage`, cuya lógica se copió 100% literal sin tocar. Ver la sección "Migrar un módulo que YA tenía service previo" más arriba.
...
- [x] ~~`src/modules/inventory/components/TabMovements.tsx` — Inventario, Movimientos.~~ **Migrado en Tanda 3g** (06/09/2026). Con `branchId` — pero el campo NO EXISTÍA en `InventoryMovement` antes de esta tanda, tuvo que agregarse al tipo y poblarse en el mock (primera migración que amplía el dominio en vez de solo reorganizarlo). `api/` en `modules/inventory/api/movements/` (dominio exclusivo de `inventory`, confirmado por `grep`). Ver la sección "Migrar una entidad a la que hay que agregarle un campo de scope" más abajo.
- [x] ~~`src/modules/inventory/components/TabProductHistory.tsx` — Inventario, Historial del Producto.~~ **Migrado en Tanda 3g**, junto con Movimientos (mismo hallazgo: sin `branchId` en el tipo). `api/` en `modules/inventory/api/product-history/`, subcarpeta SEPARADA de `movements/` pese a ser el mismo módulo — dos vistas sin relación modelada real (aprendizaje 8). Búsqueda existente (SKU/nombre) migrada a server-side con debounce.
...
**Verificación funcional en navegador PENDIENTE (del propio piloto `suppliers` que esta guía usa como plantilla) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_0_1.md`.** El cambio de Tanda 2 (cache/dedupe/invalidación vía TanStack Query, transparente para esta guía) tiene su propio checklist, también pendiente — ver `docs/historial/verificaciones/VERIFICACION_TANDA_2.md`. Lo mismo para Tanda 2.5 (`useCachedQuery`, httpClient unificado) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_2_5.md`. Y para Tanda 3a (`orders`, primer módulo sin service previo) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3A.md`. Y para Tanda 3b (`cash`, segundo módulo sin service previo) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3B.md`. Y para Tanda 3c (3 vistas de `settings`) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3C.md`. Y para Tanda 3d (Directorio de `clients`) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3D.md`. Y para Tanda 3e (Stock Actual de `inventory` + capa `api/` de productos) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3E.md`. Y para Tanda 3g (Movimientos + Historial del Producto de `inventory`) — ver `docs/historial/verificaciones/VERIFICACION_TANDA_3G.md`.
