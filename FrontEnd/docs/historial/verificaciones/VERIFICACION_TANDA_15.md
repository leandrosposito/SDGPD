...
Se revisaron TODOS los consumidores de `withIdempotency` del proyecto (`trips.service.ts`, `deliveries.service.ts`, `drivers.service.ts`, `vehicles.service.ts` — 13 usos en total): los otros 9 (`createTrip`, `assignDeliveriesToStop`, `transitionTrip`, `registerPod`, `transitionDelivery`, `createDriver`, `updateDriver`, `toggleDriverActivo`, `createVehicle`, `updateVehicle`, `toggleVehicleActivo`) YA tenían las precondiciones adentro — no se tocaron.
