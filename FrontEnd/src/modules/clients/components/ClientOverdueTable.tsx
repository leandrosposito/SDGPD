...
export const ClientOverdueTable: FC<ClientOverdueTableProps> = ({ search }) => {
  ...
  const urlState = useUrlListState<never, 'preset' | 'from' | 'to' | 'bucket'>({
    prefix: 'over',
    filterKeys: ['preset', 'from', 'to', 'bucket'],
  });

  const bucketFilter: BucketFilter =
    urlState.filters.bucket && AGING_BUCKET_ORDER.includes(urlState.filters.bucket as AgingBucket)
      ? (urlState.filters.bucket as AgingBucket)
      : 'all';
